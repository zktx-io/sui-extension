...
# MystenLabs Example: Trusted Swap

Atomic swap via a trusted custodian using **object wrapping**:
- Users submit a \`SwapRequest\` with their object + fee
- Custodian matches two compatible requests and executes the swap
- Fees are aggregated and returned to the custodian

Source: https://github.com/MystenLabs/sui/tree/main/examples/move/trusted_swap
`);
...
export const MoveTemplate_Mysten_TrustedSwap: MoveTemplate = {
  id: 'mysten_trusted_swap',
  label: 'MystenLabs: Trusted Swap',
  defaultName: 'trusted_swap',
  description: 'Third-party swap',
  detail:
    'Custodian matches requests by scarcity/style, swaps objects, and collects fees.',
  files,
};
