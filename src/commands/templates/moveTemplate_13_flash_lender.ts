...
# Flash Lender Example (Mysten Labs)

A generic flash-loan lender that works with **any \`Coin<T>\`**:
- \`new\`: Create shared \`FlashLender<T>\` with a flat \`fee\`
- \`loan\`: Borrow and receive a hot-potato \`Receipt<T>\`
- \`repay\`: Repay \`amount + fee\` in the same PTB
- Admin ops: \`withdraw\`, \`deposit\`, \`update_fee\`
- Accessors: \`fee\`, \`max_loan\`, \`repay_amount\`, \`flash_lender_id\`

Includes a full test that mints SUI, borrows, repays, and withdraws profit.

Source: https://github.com/MystenLabs/sui/tree/main/examples/move/flash_lender
`);
...
export const MoveTemplate_Mysten_FlashLender: MoveTemplate = {
  id: 'mysten_flash_lender',
  label: 'MystenLabs: Flash Lender',
  defaultName: 'flash_lender',
  description: 'Generic flash loans',
  detail:
    'Shared lender, hot-potato receipt, repay-in-PTB, and admin controls.',
  files,
};
