...
# Hero Game Example (Mysten Labs)

A small RPG-like example featuring:
- Core types: \`Hero\`, \`Sword\`, \`Potion\`, \`Boar\`, \`Game\`, \`Admin\`
- Gameplay: buy sword, create hero, slay boars, gain XP, heal, equip/unequip
- Admin: create game (shared), spawn boars/potions, collect payments
- Events: \`BoarSlainEvent\`
- Comprehensive tests covering success and expected-failure cases

Source: https://github.com/MystenLabs/sui/tree/main/examples/move/hero
`);
...
export const MoveTemplate_Mysten_Hero: MoveTemplate = {
  id: 'mysten_hero',
  label: 'MystenLabs: Hero',
  defaultName: 'hero',
  description: 'RPG-style demo',
  detail:
    'Hero, Sword, Potion, Boar, Game, Admin, gameplay flows and test coverage.',
  files,
};
